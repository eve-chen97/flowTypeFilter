...
/*****************************************************************************
 * Determine whether an event falls within a given filter gate
 *
 ****************************************************************************/

bool is_in_filter(const double data, const vector<double> &thresholds, const int segment)
{
    if (segment == 1)
    {
    ...
    {
    throw "Cannot split data to more than 2, when polygine filter is provided";
    }
    throw "can not happen!";
}
